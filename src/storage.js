...
export const ContractState = {
  get: () => {
    const contractStateStr = localStorage[`contractState`];
    return contractStateStr ? JSON.parse(contractStateStr) : {};
  },
  set: (contractState) => {
  ...
  update: (contractState) => {
    const now = GameData.get();
    localStorage.setItem(`contractState`, JSON.stringify(Object.assign(now, contractState)));
  },
  clear: () => {
    localStorage.setItem(`contractState`, JSON.stringify({}));
  },
};
